 ...
 */
package com.google.android.exoplayer2internal.offline;
...
import com.google.android.exoplayer2internal.database.DatabaseProvider;
import com.google.android.exoplayer2internal.scheduler.Requirements;
import com.google.android.exoplayer2internal.scheduler.RequirementsWatcher;
import com.google.android.exoplayer2internal.upstream.DataSource;
import com.google.android.exoplayer2internal.upstream.cache.Cache;
import com.google.android.exoplayer2internal.upstream.cache.CacheDataSource;
import com.google.android.exoplayer2internal.upstream.cache.CacheEvictor;
import com.google.android.exoplayer2internal.upstream.cache.NoOpCacheEvictor;
...
public final class DownloadManager {
   ...
   * @deprecated Use {@link #DownloadManager(Context, DatabaseProvider, Cache, DataSource.Factory, Executor)}.
   */
  @Deprecated
  public DownloadManager(
      Context context, DatabaseProvider databaseProvider, Cache cache, DataSource.Factory upstreamFactory) {
  ...
  /* package */ static Download mergeRequest(
    ...
    // existing download.
    long startTimeMs =
        state == Download.STATE_REMOVING || download.isTerminalState() ? nowMs : download.startTimeMs;
    if (state == Download.STATE_REMOVING || state == Download.STATE_RESTARTING) {
      state = Download.STATE_RESTARTING;
    } else if (stopReason != Download.STOP_REASON_NONE) {
  ...
  private static final class InternalHandler extends Handler {
      ...
      try {
        ...
        cursor =
            downloadIndex.getDownloads(
                Download.STATE_QUEUED, Download.STATE_STOPPED, Download.STATE_DOWNLOADING, Download.STATE_REMOVING, Download.STATE_RESTARTING);
    ...
    private void setStopReason(Download download, int stopReason) {
      if (stopReason == Download.STOP_REASON_NONE) {
        if (download.state == Download.STATE_STOPPED) {
          putDownloadWithState(download, Download.STATE_QUEUED);
        }
      } else if (stopReason != download.stopReason) {
        @Download.State int state = download.state;
        if (state == Download.STATE_QUEUED || state == Download.STATE_DOWNLOADING) {
          state = Download.STATE_STOPPED;
        }
        putDownload(
            new Download(
    ...
    private void addDownload(DownloadRequest request, int stopReason) {
      ...
      if (download != null) {
        ...
        putDownload(
            new Download(
                request,
                stopReason != Download.STOP_REASON_NONE ? Download.STATE_STOPPED : Download.STATE_QUEUED,
                /* startTimeMs= */ nowMs,
                /* updateTimeMs= */ nowMs,
                /* contentLength= */ C.LENGTH_UNSET,
                stopReason,
    ...
    private void removeDownload(String id) {
      ...
      putDownloadWithState(download, Download.STATE_REMOVING);
      syncTasks();
    }

    private void removeAllDownloads() {
      List<Download> terminalDownloads = new ArrayList<>();
      try (DownloadCursor cursor = downloadIndex.getDownloads(Download.STATE_COMPLETED, Download.STATE_FAILED)) {
        while (cursor.moveToNext()) {
      ...
      for (int i = 0; i < downloads.size(); i++) {
        downloads.set(i, copyDownloadWithState(downloads.get(i), Download.STATE_REMOVING));
      }
      for (int i = 0; i < terminalDownloads.size(); i++) {
        downloads.add(copyDownloadWithState(terminalDownloads.get(i), Download.STATE_REMOVING));
      }
    ...
    private Task syncQueuedDownload(@Nullable Task activeTask, Download download) {
      ...
      download = putDownloadWithState(download, Download.STATE_DOWNLOADING);
    ...
    private void onDownloadTaskStopped(Download download, @Nullable Exception finalException) {
      download =
          new Download(
              download.request,
              finalException == null ? Download.STATE_COMPLETED : Download.STATE_FAILED,
              download.startTimeMs,
              /* updateTimeMs= */ System.currentTimeMillis(),
              download.contentLength,
              download.stopReason,
              finalException == null ? Download.FAILURE_REASON_NONE : Download.FAILURE_REASON_UNKNOWN,
              download.progress);
    ...
    private void onRemoveTaskStopped(Download download) {
      if (download.state == Download.STATE_RESTARTING) {
        putDownloadWithState(
            download, download.stopReason == Download.STOP_REASON_NONE ? Download.STATE_QUEUED : Download.STATE_STOPPED);
        syncTasks();
      } else {
    ...
    private Download putDownloadWithState(Download download, @Download.State int state) {
      ...
      Assertions.checkState(
          state != Download.STATE_COMPLETED && state != Download.STATE_FAILED && state != Download.STATE_STOPPED);
    ...
    private Download putDownload(Download download) {
      ...
      Assertions.checkState(download.state != Download.STATE_COMPLETED && download.state != Download.STATE_FAILED);
