...
import com.google.android.exoplayer2internal.upstream.DataSink;
import com.google.android.exoplayer2internal.upstream.DataSource;
import com.google.android.exoplayer2internal.upstream.DataSourceException;
import com.google.android.exoplayer2internal.upstream.DataSpec;
import com.google.android.exoplayer2internal.upstream.DummyDataSource;
import com.google.android.exoplayer2internal.upstream.FileDataSource;
import com.google.android.exoplayer2internal.upstream.PriorityDataSource;
import com.google.android.exoplayer2internal.upstream.TeeDataSource;
...
public final class CacheDataSource implements DataSource {
  ...
  public int read(byte[] buffer, int offset, int readLength) throws IOException {
    DataSpec requestDataSpec = Assertions.checkNotNull(this.requestDataSpec);
    if (readLength == 0) {
      return 0;
    }
    if (bytesRemaining == 0) {
    ...
    try {
      ...
      int bytesRead = Assertions.checkNotNull(currentDataSource).read(buffer, offset, readLength);
      if (bytesRead != C.RESULT_END_OF_INPUT) {
        ...
        setNoBytesRemainingAndMaybeStoreLength(Util.castNonNull(requestDataSpec.key));
  ...
  private void openNextSource(DataSpec requestDataSpec, boolean checkCache) throws IOException {
    ...
    String key = Util.castNonNull(requestDataSpec.key);
    if (currentRequestIgnoresCache) {
      nextSpan = null;
    ...
    if (nextSpan == null) {
      ...
      Uri fileUri = Uri.fromFile(Util.castNonNull(nextSpan.file));
    ...
    if (checkCache) {
      ...
      try {
        ...
        if (Util.castNonNull(nextSpan).isHoleSpan()) {
