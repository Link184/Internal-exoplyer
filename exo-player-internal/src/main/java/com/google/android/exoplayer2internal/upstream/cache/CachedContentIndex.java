...
import com.google.android.exoplayer2internal.database.DatabaseIOException;
import com.google.android.exoplayer2internal.database.DatabaseProvider;
import com.google.android.exoplayer2internal.database.VersionTable;
import com.google.android.exoplayer2internal.util.Assertions;
import com.google.android.exoplayer2internal.util.AtomicFile;
import com.google.android.exoplayer2internal.util.ReusableBufferedOutputStream;
...
/* package */ class CachedContentIndex {
  ...
  public CachedContentIndex(
    ...
    if (databaseStorage == null || (legacyStorage != null && preferLegacyStorage)) {
      storage = Util.castNonNull(legacyStorage);
  ...
  private static class LegacyStorage implements Storage {
    ...
    public LegacyStorage(File file, @Nullable byte[] secretKey, boolean encrypt) {
      Assertions.checkState(secretKey != null || !encrypt);
    ...
    private boolean readFile(
      ...
      try {
        ...
        if ((flags & FLAG_ENCRYPTED_INDEX) != 0) {
          ...
          try {
            cipher.init(Cipher.DECRYPT_MODE, Util.castNonNull(secretKeySpec), ivParameterSpec);
    ...
    private void writeFile(HashMap<String, CachedContent> content) throws IOException {
      ...
      try {
        ...
        if (encrypt) {
          byte[] initializationVector = new byte[16];
          Util.castNonNull(random).nextBytes(initializationVector);
          output.write(initializationVector);
          IvParameterSpec ivParameterSpec = new IvParameterSpec(initializationVector);
          try {
            Util.castNonNull(cipher)
                .init(Cipher.ENCRYPT_MODE, Util.castNonNull(secretKeySpec), ivParameterSpec);
          } catch (InvalidKeyException | InvalidAlgorithmParameterException e) {
            throw new IllegalStateException(e); // Should never happen.
          }
          output.flush();
  ...
  private static final class DatabaseStorage implements Storage {
    ...
    public boolean exists() throws DatabaseIOException {
      return VersionTable.getVersion(
              ...
              Assertions.checkNotNull(hexUid))
          != VersionTable.VERSION_UNSET;
    }

    @Override
    public void delete() throws DatabaseIOException {
      delete(databaseProvider, Assertions.checkNotNull(hexUid));
    }
    ...
    private void initializeTable(SQLiteDatabase writableDatabase) throws DatabaseIOException {
      VersionTable.setVersion(
          ...
          Assertions.checkNotNull(hexUid),
          TABLE_VERSION);
      dropTable(writableDatabase, Assertions.checkNotNull(tableName));
      writableDatabase.execSQL("CREATE TABLE " + tableName + " " + TABLE_SCHEMA);
    }

    private void deleteRow(SQLiteDatabase writableDatabase, int key) {
      writableDatabase.delete(
          Assertions.checkNotNull(tableName), WHERE_ID_EQUALS, new String[] {Integer.toString(key)});
    }
    ...
    private void addOrUpdateRow(SQLiteDatabase writableDatabase, CachedContent cachedContent)
      ...
      writableDatabase.replaceOrThrow(Assertions.checkNotNull(tableName), /* nullColumnHack= */ null, values);
    }
