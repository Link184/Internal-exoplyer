...
import com.google.android.exoplayer2internal.ExoPlaybackException;
...
import com.google.android.exoplayer2internal.Timeline;
import com.google.android.exoplayer2internal.source.MediaSource;
import com.google.android.exoplayer2internal.source.TrackGroup;
import com.google.android.exoplayer2internal.source.TrackGroupArray;
import com.google.android.exoplayer2internal.upstream.BandwidthMeter;
...
public final class DefaultTrackSelectorTest {
  ...
  private static final RendererCapabilities ALL_AUDIO_FORMAT_EXCEEDED_RENDERER_CAPABILITIES =
      new FakeRendererCapabilities(C.TRACK_TYPE_AUDIO, RendererCapabilities.FORMAT_EXCEEDS_CAPABILITIES);
  ...
  private DefaultTrackSelector.Parameters defaultParameters;
  ...
  public void setUp() {
    ...
    defaultParameters = DefaultTrackSelector.Parameters.getDefaults(context);
  ...
  public void parameters_buildUponThenBuild_isEqual() {
    DefaultTrackSelector.Parameters parameters = buildParametersForEqualsTest();
    assertThat(parameters.buildUpon().build()).isEqualTo(parameters);
  }

  /** Tests {@link DefaultTrackSelector.Parameters} {@link android.os.Parcelable} implementation. */
  @Test
  public void parameters_parcelAndUnParcelable() {
    DefaultTrackSelector.Parameters parametersToParcel = buildParametersForEqualsTest();

    Parcel parcel = Parcel.obtain();
    parametersToParcel.writeToParcel(parcel, 0);
    parcel.setDataPosition(0);

    DefaultTrackSelector.Parameters parametersFromParcel = DefaultTrackSelector.Parameters.CREATOR.createFromParcel(parcel);
    assertThat(parametersFromParcel).isEqualTo(parametersToParcel);

    parcel.recycle();
  }

  /** Tests {@link DefaultTrackSelector.SelectionOverride}'s {@link android.os.Parcelable} implementation. */
  @Test
  public void selectionOverrideParcelable() {
    int[] tracks = new int[] {2, 3};
    DefaultTrackSelector.SelectionOverride selectionOverrideToParcel =
        new DefaultTrackSelector.SelectionOverride(/* groupIndex= */ 1, tracks);

    Parcel parcel = Parcel.obtain();
    selectionOverrideToParcel.writeToParcel(parcel, 0);
    parcel.setDataPosition(0);

    DefaultTrackSelector.SelectionOverride selectionOverrideFromParcel =
        DefaultTrackSelector.SelectionOverride.CREATOR.createFromParcel(parcel);
  ...
  public void selectTracksWithNullOverride() throws ExoPlaybackException {
    ...
    assertThat(result.rendererConfigurations)
        .isEqualTo(new RendererConfiguration[] {null, RendererConfiguration.DEFAULT});
  }
  ...
  public void selectTracksWithClearedNullOverride() throws ExoPlaybackException {
    ...
    assertThat(result.rendererConfigurations)
        .isEqualTo(new RendererConfiguration[] {RendererConfiguration.DEFAULT, RendererConfiguration.DEFAULT});
  }
  ...
  public void selectTracksWithDisabledRenderer() throws ExoPlaybackException {
    ...
    assertThat(new RendererConfiguration[] {RendererConfiguration.DEFAULT, null})
  ...
  public void selectTracksWithClearedDisabledRenderer() throws ExoPlaybackException {
    ...
    TrackSelectorResult result =
        trackSelector.selectTracks(RENDERER_CAPABILITIES, TRACK_GROUPS, periodId, TIMELINE);
    assertSelections(result, TRACK_SELECTIONS);
    assertThat(new RendererConfiguration[] {RendererConfiguration.DEFAULT, RendererConfiguration.DEFAULT})
        .isEqualTo(result.rendererConfigurations);
  }
   ...
   * TrackSelector.InvalidationListener#onTrackSelectionsInvalidated()} when it's set with default values of
   * {@link DefaultTrackSelector.Parameters}.
   */
  @Test
  ...
  /**
   * Tests that track selector will call {@link TrackSelector.InvalidationListener#onTrackSelectionsInvalidated()}
   * when it's set with non-default values of {@link DefaultTrackSelector.Parameters}.
   */
  @Test
  public void setParameterWithNonDefaultParameterNotifyInvalidationListener() {
    DefaultTrackSelector.ParametersBuilder builder = defaultParameters.buildUpon().setPreferredAudioLanguage("eng");
  ...
  /**
   * Tests that track selector will not call {@link
   * TrackSelector.InvalidationListener#onTrackSelectionsInvalidated()} again when it's set with the same values
   * of {@link DefaultTrackSelector.Parameters}.
   */
  @Test
   ...
   * default values of {@link DefaultTrackSelector.Parameters}.
   */
  @Test
   ...
   * given by {@link DefaultTrackSelector.Parameters}.
   */
  @Test
   ...
   * language given by {@link DefaultTrackSelector.Parameters} over track with {@link C#SELECTION_FLAG_DEFAULT}.
   */
  @Test
   ...
   * there are no other choice, given the default {@link DefaultTrackSelector.Parameters}.
   */
  @Test
   ...
   * exceed that renderer's capabilities when {@link DefaultTrackSelector.Parameters} does not allow
  ...
  /**
   * Tests that track selector will prefer tracks that are within renderer's capabilities over track
   * that have language matching preferred audio given by {@link DefaultTrackSelector.Parameters} but exceed renderer's
   * capabilities.
   */
  @Test
  ...
  /**
   * Tests that track selector will prefer tracks that are within renderer's capabilities over track
   * that have both language matching preferred audio given by {@link DefaultTrackSelector.Parameters} and {@link
   ...
   * DefaultTrackSelector.Parameters#selectUndeterminedTextLanguage} is true.
   */
  @Test
  public void selectUndeterminedTextLanguageAsFallback() throws ExoPlaybackException {
    ...
    DefaultTrackSelector.ParametersBuilder builder = defaultParameters.buildUpon().setPreferredTextLanguage("spa");
    trackSelector.setParameters(builder);
    result = trackSelector.selectTracks(textRendererCapabilites, trackGroups, periodId, TIMELINE);
   ...
   * DefaultTrackSelector.Parameters#forceLowestBitrate} is set.
   */
  @Test
  public void selectTracksWithinCapabilitiesAndForceLowestBitrateSelectLowerBitrate()
    ...
    Map<String, Integer> mappedCapabilities = new HashMap<>();
    mappedCapabilities.put(unsupportedLowBitrateFormat.id, RendererCapabilities.FORMAT_EXCEEDS_CAPABILITIES);
    mappedCapabilities.put(lowerBitrateFormat.id, RendererCapabilities.FORMAT_HANDLED);
    mappedCapabilities.put(higherBitrateFormat.id, RendererCapabilities.FORMAT_HANDLED);
    RendererCapabilities mappedAudioRendererCapabilities =
        new FakeMappedRendererCapabilities(C.TRACK_TYPE_AUDIO, mappedCapabilities);
   ...
   * DefaultTrackSelector.Parameters#forceHighestSupportedBitrate} is set.
   */
  @Test
  public void selectTracksWithinCapabilitiesAndForceHighestBitrateSelectHigherBitrate()
    ...
    mappedCapabilities.put(lowerBitrateFormat.id, RendererCapabilities.FORMAT_HANDLED);
    mappedCapabilities.put(higherBitrateFormat.id, RendererCapabilities.FORMAT_HANDLED);
    mappedCapabilities.put(exceedsBitrateFormat.id, RendererCapabilities.FORMAT_EXCEEDS_CAPABILITIES);
    RendererCapabilities mappedAudioRendererCapabilities =
        new FakeMappedRendererCapabilities(C.TRACK_TYPE_AUDIO, mappedCapabilities);
    ...
    trackSelector.setParameters(
        trackSelector
            ...
            .setSelectionOverride(
                /* rendererIndex= */ 0,
                trackGroups,
                new DefaultTrackSelector.SelectionOverride(/* groupIndex= */ 0, /* tracks=... */ 1, 2)));
    TrackSelectorResult result =
        trackSelector.selectTracks(
            new RendererCapabilities[] {AUDIO_CAPABILITIES}, trackGroups, periodId, TIMELINE);
  ...
  public void selectTracksWithMultipleVideoTracksWithNonSeamlessAdaptiveness() throws Exception {
    FakeRendererCapabilities nonSeamlessVideoCapabilities =
        new FakeRendererCapabilities(C.TRACK_TYPE_VIDEO, RendererCapabilities.FORMAT_HANDLED | RendererCapabilities.ADAPTIVE_NOT_SEAMLESS);
  ...
  /**
   * Returns {@link DefaultTrackSelector.Parameters} suitable for simple round trip equality tests.
   *
   ...
   * variables.
   */
  private static DefaultTrackSelector.Parameters buildParametersForEqualsTest() {
    SparseArray<Map<TrackGroupArray, DefaultTrackSelector.SelectionOverride>> selectionOverrides = new SparseArray<>();
    Map<TrackGroupArray, DefaultTrackSelector.SelectionOverride> videoOverrides = new HashMap<>();
    videoOverrides.put(new TrackGroupArray(VIDEO_TRACK_GROUP), new DefaultTrackSelector.SelectionOverride(0, 1));
    ...
    return new DefaultTrackSelector.Parameters(
        // Video
        /* maxVideoWidth= */ 0,
        /* maxVideoHeight= */ 1,
        /* maxVideoFrameRate= */ 2,
